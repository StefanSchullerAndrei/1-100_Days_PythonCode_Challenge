...
# Describe Problem
# range function will not work because the upper limit is not taken in consideration
...
# Play Computer
# ### use 1994 as the input => error because equal to 1994 is not represented
...
### == means it will be evaluated , = means it will assign the new value
...
# b_list.append - same indentation as new_item is required so it will add all items to the empty list.
def mutate(a_list):
  b_list = []
  for item in a_list:
    new_item = item * 2
